...
UPDATE "USER" SET role = 'COACH' WHERE role = 'USER' AND name IN ('李燕容', '肌肉棒子', 'Q太郎');

-- 1-3 刪除：刪除USER 資料表中，用 Email 找到透明人，並刪除該筆資料
DELETE FROM "USER" WHERE name = '透明人';
...
SELECT u.id, cp.id, cp.credit_amount, cp.price 
FROM "USER" AS u 
JOIN "CREDIT_PACKAGE" AS cp 
ON (
    (u.name = '王小明' AND cp.name = '14 堂組合包方案')
    OR (u.name = '王小明' AND cp.name = '21 堂組合包方案')
    OR (u.name = '好野人' AND cp.name = '14 堂組合包方案')
);
...
JOIN "SKILL" AS s ON (
    (u.name = '肌肉棒子' AND s.name = '瑜伽')
    OR (u.name = 'Q太郎' AND s.name = '有氧運動')
    OR (u.name = 'Q太郎' AND s.name = '復健訓練')
);

-- INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
-- SELECT c.id, s.id
-- FROM "COACH" AS c, "SKILL" AS s 
-- WHERE s.name = '重訓'
-- UNION ALL
-- SELECT c.id, s.id
-- FROM "COACH" AS c, "SKILL" AS s, "USER" AS u
...
UPDATE "COACH" SET experience_years = 3 WHERE user_id IN (SELECT id FROM "USER" WHERE name = '肌肉棒子');
UPDATE "COACH" SET experience_years = 5 WHERE user_id IN (SELECT id FROM "USER" WHERE name = 'Q太郎');
...
--INSERT INTO "COURSE_BOOKING" (user_id, booking_at,)
